  ...
  <div class="accommodation-request">
    ...
    <div class="main-content">
      ...
      <div class="request-form-card">
        <h2>Creating an accommodation request</h2>
      ...
      <div class="passengers-card">
        <h2>Choose passengers</h2>
        
        <div class="passengers-grid">
          <!-- Team Members -->
          <div v-for="member in teamMembers" :key="`team-${member.idTeam}-${member.idPlayer}`" class="passenger-item">
          ...
          <!-- Management Members -->
          <div v-for="manager in managementMembers" :key="`mgmt-${manager.memberId}`" class="passenger-item">
            <label class="passenger-checkbox">
              <input 
                type="checkbox" 
      ...
      <div class="send-request-section">
        <button class="send-request-btn" @click="openAgencyModal" :disabled="isLoading">
    ...
    <div v-if="showAgencyModal" class="modal-overlay" @click="closeAgencyModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>Select Accommodation Agencies</h3>
          <button class="close-btn" @click="closeAgencyModal">&times;</button>
        </div>
        
        <div class="modal-body">
          <div class="agency-list">
            <div v-for="agency in accommodationAgencies" :key="agency.id" class="agency-item">
...
export default {
  ...
  methods: {
    onCheckInDateChange() {
      ...
      if (this.formData.checkOutDate && this.formData.checkInDate && this.formData.checkOutDate < this.formData.checkInDate) {
        this.formData.checkOutDate = this.formData.checkInDate
      }
    },
    getUserId() {
    ...
    async loadData() {
      ...
      try {
        await Promise.all([
          ...
          this.loadAccommodationAgencies()
        ])
      } catch (error) {
    ...
    async loadManagementMembers() {
      try {
        ...
        if (response.data.isSuccess) {
          this.managementMembers = response.data.value
        }
      } catch (error) {
    ...
    async loadAccommodationAgencies() {
      try {
        const response = await axios.get('https://localhost:5007/api/agencies/accommodation')
        this.accommodationAgencies = Array.isArray(response.data.value.agencies)
          ? response.data.value.agencies
          : []
      } catch (error) {
        console.error('Error loading accommodation agencies:', error)
    ...
    validateForm() {
      if (!this.formData.numberOfGuests || !this.formData.numberOfRooms || 
          !this.formData.checkInDate || !this.formData.checkOutDate || 
          !this.formData.state || !this.formData.city || !this.formData.hall ||
          !this.formData.accommodationType || !this.formData.budget) {
      ...
      if (this.formData.checkInDate < today) {
        alert('Check-in date cannot be in the past')
        return false
      }

      if (this.formData.checkOutDate < this.formData.checkInDate) {
        alert('Check-out date must be the same as or after check-in date')
        return false
      }
    ...
    async sendRequest() {
      ...
      try {
        const requestData = {
          ...
          numberOfRooms: this.formData.numberOfRooms,
          checkInDate: this.formData.checkInDate.split('T')[0],
        ...
        alert('Failed to create accommodation request')
      } finally {
        this.isLoading = false
      }
    },
    ...
    navigateToTab(tab) {
      switch(tab) {
        ...
        case 'accommodationArchive':
          ...
          break
        // accommodationRequest tab is already active
      }
    }
  }
}
