      ...
      <div class="container">
        <div class="header-content">
          <nav class="nav-menu" v-if="isTeamManager">
            <router-link to="/team-manager/matches" class="nav-link">Matches</router-link>
            <router-link to="/team-manager/players" class="nav-link active">Players</router-link>
    ...
    <div class="container">
      <div class="player-overview">
        ...
        <div class="overview-actions">
          <button class="btn btn-primary" @click="openAddNoteModal" :disabled="loading">+ New Player Note</button>
        </div>
          ...
          <div v-for="player in filteredPlayers" :key="player.id" class="player-card">
            <div class="card-header">
              ...
              <div class="card-actions">
                <button class="btn btn-secondary btn-edit" @click="openEditNoteModal(player)" :disabled="loading">Edit</button>
                </div>
            </div>
            <div class="card-body">
      ...
      <div v-if="showNoteModal" class="modal-overlay" @click="closeNoteModal">
        <div class="modal" @click.stop>
          <div class="modal-header">
            <h3>{{ isEditMode ? 'Edit Player Note' : 'Add New Player Note' }}</h3>
            <button v-if="isEditMode" class="btn btn-secondary add-visa-btn" @click="openVisaModal">Add Visa</button>
          </div>
          <div class="modal-body">
            <div class="form-group">
            <label>Player: *</label>
            <select v-model="editNoteData.playerId" class="form-select" :disabled="isEditMode" required>
                <option value="">Select player</option>
                <option v-for="player in teamPlayers" :key="player.idPlayer" :value="player.idPlayer">
                {{ player.fullName }}
                </option>
            </select>
            </div>
            <div class="form-group">
              ...
              <input type="text" v-model="editNoteData.passportNumber" class="form-control" required />
            </div>
            <div class="form-group">
              <label>Passport Expiry Date: *</label>
              <input type="date" v-model="editNoteData.passportExpirationDate" class="form-control" required />
            </div>
            <div class="form-group">
              <label>Phone Number: *</label>
              <input type="text" v-model="editNoteData.phone" class="form-control" required />
            </div>
            <div class="form-group">
              <label>Email Address: *</label>
              <input type="email" v-model="editNoteData.email" class="form-control" required />
            </div>
          </div>
          <div class="modal-actions">
            <button class="btn btn-secondary" @click="closeNoteModal" :disabled="loading">Cancel</button>
            <button class="btn btn-primary" @click="isEditMode ? editNote() : addNote()" :disabled="loading">
              {{ loading ? 'Processing...' : (isEditMode ? 'Edit Note' : 'Add Note') }}
            </button>
          </div>
        </div>
      </div>
      ...
      <!-- Add Visa Modal -->
      <div v-if="showVisaModal" class="modal-overlay" @click="closeVisaModal">
        <div class="modal" @click.stop>
          <div class="modal-header">
            <h3>Add Visa</h3>
          </div>
          <div class="modal-body">
            <div class="form-group">
              <label>Place:</label>
              <select v-model="visaData.place" class="form-select">
            ...
            <div class="form-group">
              ...
              <input type="text" v-model="visaData.number" class="form-control" />
            </div>
            <div class="form-group">
              <label>Date of Issue:</label>
              <input type="date" v-model="visaData.issueDate" class="form-control" />
            </div>
            <div class="form-group">
              <label>Expiry Date:</label>
              <input type="date" v-model="visaData.expiryDate" class="form-control" />
            </div>
          </div>
          <div class="modal-actions">
            <button class="btn btn-primary" @click="addVisa" :disabled="loading">
              {{ loading ? 'Adding...' : 'Add Visa' }}
            </button>
          </div>
        </div>
      </div>
...
const addNote = async () => {
  ...
  if (!editNoteData.value.playerId) {
    alert('Please select a player');
    return;
  }
  if (!editNoteData.value.passportNumber) {
    alert('Passport number is required');
    return;
  }
  if (!editNoteData.value.passportExpirationDate) {
    alert('Passport expiration date is required');
    return;
  }
  if (!editNoteData.value.phone) {
    alert('Phone number is required');
    return;
  }
  if (!editNoteData.value.email) {
  ...
  try {
    loading.value = true;
    const payload = {
      IdPlayer: parseInt(editNoteData.value.playerId), 
      PassportNumber: editNoteData.value.passportNumber,
      // send ISO date string (YYYY-MM-DD) which model binder maps to DateOnly
      PassportExpirationDate: editNoteData.value.passportExpirationDate,
      Phone: editNoteData.value.phone,
      Email: editNoteData.value.email,
      Role: "player", 
    ...
    // If validation errors exist, aggregate them
    if (resp?.errors && typeof resp.errors === 'object') {
      const msgs = []
      for (const key of Object.keys(resp.errors)) {
        msgs.push(...resp.errors[key])
      }
      alert(msgs.join('\n'))
    } else {
      alert(resp?.error || resp?.message || 'Failed to create player note')
    }
  } finally {
    loading.value = false;
  }
...
const editNote = async () => {
  ...
  try {
    ...
    await axios.put(`https://localhost:5007/api/travelinformations/${editNoteData.value.playerId}/${userId}`, payload, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })
    ...
    if (resp?.errors && typeof resp.errors === 'object') {
      ...
      alert(resp?.error || resp?.message || 'Failed to update player note')
    }
  } finally {
    loading.value = false
  }
...
const addVisa = async () => {
  ...
  // Validation
  if (!visaData.value.number) {
    alert('Visa number is required')
    return
  }
  if (!visaData.value.place) {
    alert('Place/state is required')
    return
  }
  if (!visaData.value.issueDate) {
    alert('Date of issue is required')
    return
  }
  if (!visaData.value.expiryDate) {
    alert('Expiry date is required')
    return
  }
  if (!editNoteData.value.playerId) {
    alert('No travel information selected for this visa')
    return
  }
  ...
  try {
    ...
    const payload = {
      VisaNumber: visaData.value.number,
      State: visaData.value.place,
      CreationDate: visaData.value.issueDate,
      ExpirationDate: visaData.value.expiryDate,
      IdTravelInformation: parseInt(editNoteData.value.playerId)
    }

    console.log('Creating visa with payload:', payload)

    await axios.post(`https://localhost:5007/api/visas/${userId}`, payload, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })

    await fetchVisas(editNoteData.value.playerId)
    closeVisaModal()
    // reset local visa form
    visaData.value = { place: '', number: '', issueDate: '', expiryDate: '' }
  } catch (error) {
    ...
    if (resp?.errors && typeof resp.errors === 'object') {
      const msgs = []
      for (const key of Object.keys(resp.errors)) msgs.push(...resp.errors[key])
      alert(msgs.join('\n'))
    } else {
      alert(resp?.error || resp?.message || 'Failed to add visa')
    }
  } finally {
    loading.value = false
  }
