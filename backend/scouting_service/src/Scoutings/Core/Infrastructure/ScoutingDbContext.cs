...
public partial class ScoutingDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Season>(entity =>
        {
            ...
            entity.HasMany(d => d.IdMetrics).WithMany(p => p.IdSeasons)
                .UsingEntity<Dictionary<string, object>>(
                    "SeasonMetric",
                    r => r.HasOne<Metric>().WithMany()
                        .HasForeignKey("IdMetrics")
                        .OnDelete(DeleteBehavior.ClientSetNull)
                        .HasConstraintName("fk_season_metrics_metrics"),
                    l => l.HasOne<Season>().WithMany()
                        .HasForeignKey("IdSeason")
                        .OnDelete(DeleteBehavior.ClientSetNull)
                        .HasConstraintName("fk_season_metrics_season"),
                    j =>
                    {
                        j.HasKey("IdSeason", "IdMetrics").HasName("season_metrics_pkey");
                        j.ToTable("season_metrics");
                        j.IndexerProperty<int>("IdSeason").HasColumnName("id_season");
                        j.IndexerProperty<int>("IdMetrics").HasColumnName("id_metrics");
                    });
        });
